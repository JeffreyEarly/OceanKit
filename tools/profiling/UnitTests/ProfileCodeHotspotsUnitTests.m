classdef ProfileCodeHotspotsUnitTests < matlab.unittest.TestCase
    ...
    methods (Test)
        ...
        function compareProfileHotspotsComputesDeltasAndContext(testCase)
            ...
            changedLineRegressionRows = comparison.regressions( ...
                comparison.regressions.targetType == "line" & comparison.regressions.changeType == "changed", :);
            newLineRegressionRows = comparison.regressions( ...
                comparison.regressions.targetType == "line" & comparison.regressions.changeType == "new", :);
            ...
            testCase.verifyEqual(comparison.regressions.targetType(1), "function")
            ...
            testCase.verifyFalse(isempty(changedLineRegressionRows))
            testCase.verifyTrue(all(contains(changedLineRegressionRows.reason, "code-motion-sensitive")))
            testCase.verifyFalse(isempty(newLineRegressionRows))
            testCase.verifyTrue(all(contains(newLineRegressionRows.reason, "code motion")))
            testCase.verifyTrue(any(contains(comparison.summary, "Function-level totals are the primary comparison signal.")))
            testCase.verifyNotEmpty(comparison.summary)
        end

        function compareProfileHotspotsWarnsWhenOnlyLineDiffsChange(testCase)
            projectRoot = "/tmp/project";
            baseline = analyzeProfileInfo(struct("FunctionTable", ...
                ProfileCodeHotspotsUnitTests.functionEntry( ...
                    completeName="pkg.foo", functionName="foo", ...
                    fileName=projectRoot + "/foo.m", type="M-function", ...
                    totalTime=4, numCalls=2, children=[], parents=[], ...
                    executedLines=[10 1 1], isRecursive=false)), ...
                projectRoots=projectRoot, label="before");
            candidate = analyzeProfileInfo(struct("FunctionTable", ...
                ProfileCodeHotspotsUnitTests.functionEntry( ...
                    completeName="pkg.foo", functionName="foo", ...
                    fileName=projectRoot + "/foo.m", type="M-function", ...
                    totalTime=4, numCalls=2, children=[], parents=[], ...
                    executedLines=[10 1 3], isRecursive=false)), ...
                projectRoots=projectRoot, label="after");

            comparison = compareProfileHotspots(baseline, candidate);

            testCase.verifyEqual(comparison.regressions.targetType(1), "line")
            testCase.verifyTrue(contains(comparison.regressions.reason(1), "code-motion-sensitive"))
            testCase.verifyTrue(any(contains(comparison.summary, "code-motion-sensitive")))
        end
