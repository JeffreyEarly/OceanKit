...
- Direct cross-package runtime calls are reflected in `resources/mpackage.json`.
- Dependency floors cover the first released version that provides newly required
  symbols or behavior.
- Release verification includes an OceanKit snapshot install or clean-path test,
  not only sibling authoring repositories.
